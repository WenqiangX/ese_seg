...
__all__ = ['DarknetV3', 'get_darknet', 'darknet53', 'Tiny_darknet']
...
def Conv2dBatchLeaky(in_channels, out_channels, kernel, stride, padding, norm_layer=BatchNorm, norm_kwargs=None):
    """A common conv-bn-leakyrelu cell"""
    cell = nn.HybridSequential(prefix='')
...
class TinyYolov3(gluon.HybridBlock):
    ...
    def __init__(self, classes=1000, **kwargs):
        ...
        with self.features.name_scope():
                self.features.add(Conv2dBatchLeaky(3, 16, 3, 1, 1))  # 416->416
                self.features.add(nn.MaxPool2D(2, 2))                # 416->208
                self.features.add(Conv2dBatchLeaky(16, 32, 3, 1, 1))  # 208->208
                self.features.add(nn.MaxPool2D(2, 2))                # 208->104
                self.features.add(Conv2dBatchLeaky(32, 16, 3, 1, 1))  # 104->104

                self.features.add(Conv2dBatchLeaky(16, 128, 3, 1, 1))  # 104->104
                self.features.add(Conv2dBatchLeaky(128, 64, 3, 1, 1))  # 104->104
                
                self.features.add(nn.MaxPool2D(2, 2))  # 104->52
                self.features.add(Conv2dBatchLeaky(64, 128, 3, 1, 1))  # 52->52
            
                self.features.add(nn.MaxPool2D(2, 2))    # 52->26
                self.features.add(Conv2dBatchLeaky(128, 256, 3, 1, 1))  # 26->26
                
                self.features.add(nn.MaxPool2D(2, 2))  # 26->13
                self.features.add(Conv2dBatchLeaky(256, 512, 3, 1, 1))  # 13->13
                self.features.add(nn.MaxPool2D(3, 1, 1))  # 13->13
                self.features.add(Conv2dBatchLeaky(512, 1024, 3, 1, 1))  # 13->13
                self.features.add(Conv2dBatchLeaky(1024, 256, 1, 1, 0))  # 13->13
...
def Tiny_darknet():
    ...
    Reference: Imitate the impletation of TencentYoutu  'https://github.com/TencentYoutuResearch/ObjectDetection-OneStageDet' 
    Parameters
    ----------
    Returns
    -------
