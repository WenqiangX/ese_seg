...
class YOLOV3PrefetchTargetGenerator(gluon.Block):
    ...
    """
    def __init__(self, num_class,deg=8, **kwargs):
    ...
    def forward(self, img, xs, anchors, offsets, gt_boxes,gt_coef_centers, gt_coef, gt_ids, gt_mixratio=None):
        ...
        with autograd.pause():
            ...
            for b in range(matches.shape[0]):
                for m in range(matches.shape[1]):
                    ...
                    gtcoef = np_coef[b, m ,:]
                    gtcoef[0] = gtcoef[0] - 0.263289  # recover the first che coef
        ...
        return objectness, center_targets, scale_targets, coef_center_targets, coef_targets, weights, class_targets
...
class YOLOV3DynamicTargetGeneratorSimple(gluon.HybridBlock):
    ...
    """
    def __init__(self, num_class, ignore_iou_thresh, deg=8,**kwargs):
    ...
    def hybrid_forward(self, F, box_preds, gt_boxes):
        ...
        return objness_t, center_t, scale_t, coef_center_t, coef_t, weight_t, class_t
...
class YOLOV3TargetMerger(gluon.HybridBlock):
    ...
    """
    def __init__(self, num_class, ignore_iou_thresh, deg, **kwargs):
        super(YOLOV3TargetMerger, self).__init__(**kwargs)
        self._num_class = num_class
        self._dynamic_target = YOLOV3DynamicTargetGeneratorSimple(num_class, ignore_iou_thresh, deg)
        self._label_smooth = False
        self._deg = deg

    def hybrid_forward(self, F, box_preds, gt_boxes, obj_t, centers_t, scales_t, coef_center_t, coef_t, weights_t, clas_t):
        """Short summary.
        ...
        with autograd.pause():
            ...
            obj, centers, scales, coef_center, coef, weights, clas = zip(
                dynamic_t, [obj_t, centers_t, scales_t, coef_center_t, coef_t, weights_t, clas_t])
            mask = obj[1] > 0
            ...
            coef_center_targets = F.where(mask2, coef_center[1], coef_center[0])
            mask_coef = mask.tile(reps=(self._deg*2+2))
            ...
            return [F.stop_gradient(x) for x in [objectness, center_targets, scale_targets,coef_center_targets, coef_targets,
