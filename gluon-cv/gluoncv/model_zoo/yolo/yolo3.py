...
class YOLOOutputV3(gluon.HybridBlock):
    ...
    def __init__(self, index, num_class, anchors, stride,
                 alloc_size=(128, 128), deg=8, **kwargs):
        super(YOLOOutputV3, self).__init__(**kwargs)
        anchors = np.array(anchors).astype('float32')
        self._classes = num_class
        self._num_pred = 1 + 4 + num_class + 2 + deg*2 +2  # 1 objness + 4 box + num_class + pre_center + coef
    ...
    def reset_class(self, classes, reuse_weights=None):
        ...
        self._num_pred = 1 + 4 + len(classes) + 2 + self._deg*2 + 2
        all_pred = self._num_pred * self._num_anchors
    ...
    def hybrid_forward(self, F, x, anchors, offsets):
        ...
        raw_coefs = pred.slice_axis(axis=-1, begin=6, end=6+2*self._deg+2)
        objness = pred.slice_axis(axis=-1, begin=6+2*self._deg+2, end=7+2*self._deg+2)
        class_pred = pred.slice_axis(axis=-1, begin=7+2*self._deg+2, end=None)
        ...
        coefs_other = raw_coefs.slice_axis(axis=-1, begin=1 , end=2*self._deg+2)
        ...
        if autograd.is_training():
            ...
            return (bbox.reshape((0, -1, 4)), raw_box_centers, raw_box_scales, raw_coef_centers, raw_coefs, 
        ...
        detections = F.concat(ids, scores, bboxes, absolute_coef_centers, coefs,dim=-1)# r_all, dim=-1)
        # reshape to (B, xx,4+2+2+(2*deg+2)) bbox, id+scores, coef_center, coef(2*deg+2)
        detections = F.reshape(detections.transpose(axes=(1, 0, 2, 3, 4)), (0, -1, 6+2+2*self._deg+2))
        return detections
