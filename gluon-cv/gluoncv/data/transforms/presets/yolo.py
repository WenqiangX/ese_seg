...
class YOLO3DefaultTrainTransform(object):
    ...
    def __init__(self, width, height, net=None, mean=(0.485, 0.456, 0.406),
                 std=(0.229, 0.224, 0.225), mixup=False,deg = 8, **kwargs):
        self._width = width
        self._height = height
        self._mean = mean
        self._std = std
        ...
        with autograd.train_mode():
            _, self._anchors, self._offsets, self._feat_maps, _, _, _, _, _, _ = net(self._fake_x)
        from ....model_zoo.yolo.yolo_target import YOLOV3PrefetchTargetGenerator
        self._target_generator = YOLOV3PrefetchTargetGenerator(
            num_class=len(net.classes), deg = self._deg, **kwargs)
    ...
    def __call__(self, src, label):
        ...
        gt_coef = mx.nd.array(bbox[np.newaxis, :, 6:6+2*self._deg+2])
        gt_ids = mx.nd.array(bbox[np.newaxis, :, 6+2*self._deg+2:7+2*self._deg+2])
        if self._mixup:
            gt_mixratio = mx.nd.array(bbox[np.newaxis, :, -1:])
        else:
            gt_mixratio = None
        objectness, center_targets, scale_targets, coef_center_targets, coef_targets, weights, class_targets = self._target_generator(
            self._fake_x, self._feat_maps, self._anchors, self._offsets,
            gt_bboxes, gt_coef_centers, gt_coef, gt_ids, gt_mixratio)
        
        return (img, objectness[0], center_targets[0], scale_targets[0],coef_center_targets[0], coef_targets[0], weights[0],
...
class YOLO3DefaultValTransform(object):
    ...
    """
    def __init__(self, width, height, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225), deg=8):
        self._width = width
        self._height = height
        self._mean = mean
        self._std = std
